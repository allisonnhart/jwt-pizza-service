...
#   token=$(echo $response | grep -oP '"token"\s*:\s*"\K[^"]+')
#   echo "Login diner..."
#   curl -s -X POST $host/api/order -H 'Content-Type: application/json' -d '{"franchiseId": 1, "storeId":1, "items":[{ "menuId": 76, "description": "Not a pizza type", "price": 0.05 }]}'  -H "Authorization: Bearer $token" > /dev/null;
#   echo "Failed to buy a pizza..."
#   sleep 10;
#   curl -s -X DELETE $host/api/auth -H "Authorization: Bearer $token" > /dev/null;
#   echo "Logging out diner..."
